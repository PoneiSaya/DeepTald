...
class StreamHandler:
    ...
    def callback(self, indata, frames, time, status):
        ...
        if np.sqrt(np.mean(indata**2)) > Threshold and Vocals[0] <= freq <= Vocals[1] and not self.asst.talking:
            print("\033[32mRecording..\033[0m", end='', flush=True)
    ...
    def process(self):
        if self.fileready:
            ...
            result = self.model.transcribe('dictate.wav',fp16=False,language='en' if English else 'it',task='translate' if Translate else 'transcribe')
